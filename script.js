...
// 变卦映射（简化版，实际由爻变计算）
const bianGuaMap = {
  "乾卦": "姤卦", "坤卦": "复卦", "震卦": "恒卦", "巽卦": "渐卦", "坎卦": "屯卦", "离卦": "睽卦", "艮卦": "咸卦", "兑卦": "困卦",
  "夬卦": "大有卦", "需卦": "比卦", "大有卦": "夬卦", "大壮卦": "观卦", "小畜卦": "中孚卦", "泰卦": "否卦", "履卦": "同人卦", "同人卦": "履卦",
  "革卦": "兑卦", "丰卦": "家人卦", "家人卦": "丰卦", "既济卦": "未济卦", "未济卦": "既济卦", "明夷卦": "晋卦", "贲卦": "明夷卦", "剥卦": "复卦",
  "复卦": "剥卦", "颐卦": "大过卦", "大畜卦": "颐卦", "损卦": "益卦", "益卦": "损卦", "蛊卦": "蒙卦", "屯卦": "坎卦", "否卦": "泰卦",
  "观卦": "大壮卦", "噬嗑卦": "贲卦", "鼎卦": "震卦", "恒卦": "巽卦", "旅卦": "离卦", "大过卦": "鼎卦", "姤卦": "乾卦", "遁卦": "大壮卦",
  "无妄卦": "大畜卦", "讼卦": "需卦", "咸卦": "恒卦", "蹇卦": "解卦", "渐卦": "归妹卦", "谦卦": "豫卦", "困卦": "兑卦", "井卦": "巽卦",
  "晋卦": "明夷卦", "萃卦": "升卦", "升卦": "萃卦", "蒙卦": "蛊卦", "师卦": "比卦", "涣卦": "节卦", "节卦": "涣卦", "中孚卦": "小畜卦",
  "小过卦": "既济卦", "归妹卦": "渐卦", "睽卦": "旅卦", "比卦": "师卦", "豫卦": "谦卦", "随卦": "蛊卦", "临卦": "观卦"
};
...
function yaoChange(yao) {
  if (yao === "老阴") return 1; // 老阴变阳
  if (yao === "老阳") return 0; // 老阳变阴
  return yaoToBinary(yao); // 少阴少阳不变
}

// 生成卦象
function generateGua() {
  console.log("Button clicked!"); // 调试：确认按钮点击
  ...
  for (let i = 0; i < 6; i++) {
    ...
    if ((yao === "老阴" || yao === "老阳") && movingYao === -1) movingYao = i; // 记录第一个动爻
  }
  ...
  const bianGua = guaData[bianCode] ? guaData[bianCode].name : bianGuaMap[benGua] || "未知卦";
  ...
  // 生成解读
  const benDesc = guaData[benCode] ? guaData[benCode].description : guaData[Object.keys(guaData)[0]].description;
  const bianDesc = guaData[bianCode] ? guaData[bianCode].description : guaData[Object.keys(guaData)[1]].description;
  ...
  if (movingYao !== -1) {
    interpretation += `<br><strong>动爻提示：</strong>第${movingYao + 1}爻变化，象征局势的关键转折点，请特别留意。`;
  }
